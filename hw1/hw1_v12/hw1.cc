...
// }
void sortTwoArray(float* myData, float* sortData, int my_cnt, int sort_cnt, bool isRight) {
        ...
        if(rank%2 == 0) { // even index
            if(rank != size-1) {
                ...
                if(proc_data[dataPerProc-1] > buffer[0]) {
                    ...
                    sortTwoArray(proc_data, buffer, dataPerProc, rightProcDataCount, 0);
            ...
            if(proc_data[0] < buffer[0]) {
                ...
                sortTwoArray(proc_data, buffer, dataPerProc, leftProcDataCount, 1);
                // printf("rank = %d, proc data = %f\n", rank, proc_data[0]);
            }
        }
        // odd phase
